...
void read_new()
{
  ...
  string File_num[2] = {"Bg", "Co60"};
  TH1D *h1[4], *background;
  
  for(int i = 0 ; i < 2 ; i++)
    {
      //      string root_file_name = "./080671_Crystal/" + File_num[i] + "/plot.root";
      string root_file_name = "./" + File_num[i] + ".root";
      const char *rfn = root_file_name.c_str();
      
      TFile* gfile = TFile::Open(rfn); //open file
      TDirectory* dir = gFile->GetDirectory("Energy"); //locate the directory
      
      dir->GetObject("_R_EnergyCH1@DT5730_1204", h1[i]); //change the ch is you want
    }

  TH1D* h2 = new TH1D("h2", "h2", 4095, 0, 4095);
  *h2 = *h1[1] - *h1[0];
  /*
