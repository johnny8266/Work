      subroutine CROSS_4HE_RE0(xb,d2g,dq2,ebeam,eh,phideg,dlambda,
     1    crosstotre)
      IMPLICIT REAL*8(A-H,O-Z)
      
ccccccxb is the experimental Bjorken variable defined as xb = q^2/(2 M_NUCLEON nu)
ccccccdq2 is the virtuality of the initial photon. It in expressed in GeV^2
ccccccd2g is the momentum trasferred to the nuclear system. It is negative and expressed in GeV^2.
      
      phidegg = (180.d0-phideg)

      CALL IMAGINARYCFF0(xb,d2g,dq2,ris)


      CALL COHDVCSXSECTION0(xb,dq2,d2g,ebeam,eh,dlambda,
     1     phidegg,ris,0.d0,crosstotre)
...
cccccc
      ...
      SUBROUTINE IMAGINARYCFF0(XB,D2G,DQ2,DIMGPD)
...
ccccccc
       ...
       aph(2) = 2.d0*pi
       nph(1) = 20
       CALL DMPP0(Mph,NPOINTphi,Nph,Aph,phi,wphi)
cccccc      
         D2 = D2G * 1.d6 * dc*dc
...
cccccccc                    
               ...
               CALL DMPP0(MZ,NPOINTZ,NZ,AZ,Z,wWZ)
cccccccc
cccccccc                    
                  ...
                  do iz = 1, npointz   !LOOP OVER THE LIGHTCONE MOMENTUM FRACTION (Z)
                     ...
                     CALL GPDNUCLEON0(XG,XIG,D2G,1,dq2,nn,GPDU)
                     CALL GPDNUCLEON0(XG,XIG,D2G,2,dq2,nn,GPDD)
                     CALL GPDNUCLEON0(XG,XIG,D2G,3,dq2,nn,GPDS)
                     gpdsu= dks*gpds
...
ccccccccc
                        ...
                        ap(2) = 8.d0
                        np(1) = 24
                        CALL DMPP0(Mp,NPOINTp,Np,Ap,dpg,wW)
...
cccccccc
     ...
     *                              DCOST*D3  ))
                         ...
                         CONT1 = CONT1 + TILDM*DPG(IP)*DNTOTG * WW(IP)
                               ...
                               DNPEX = dnktot0(DPG(IP))-dnkgr0(DPG(IP))
     ...
     *                              DCOST*D3  ))
                               ...
                               DNPDEX = dnktot0(DPDG)-dnkgr0(DPDG)
...
cccccc
      ...
      subroutine COHDVCSXSECTION0(xb,dq2,d2g,ebeam,ehad,dlambda,phideg,
...
cccccc
        double precision function dnktot0(dkk)
      ...
      call lagm0(dnk,1,41,0.d0,0.2d0,dkn,1,1,dnkn,1)
      ...
      double precision function dnkgr0(dkk)
CCCCCCC
...
CCCCCCC
      ...
      call lagm0(dnkg,1,41,0.d0,0.2d0,dkn,1,1,dnkn,1)
...
cccccc
      ...
      SUBROUTINE LAGM0(G,J0,JF,R0,H,RIN,IN0,NIN,GIN,I0)
...
CCCCCC
      ...
      subroutine gpdnucleon0(X,XI,D2G,I,dq2,nn,GPD)
...
50       C(1) = 0.76D0 + 0.248*DL
 ...
 20     C(1) = 0.123D0 +0.0003D0*DL
      ...
      SUBROUTINE PARAREGGE0(I,nn,dq2)
      IMPLICIT REAL*8(A-H,O-Z)
...
c     *--*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        ...
        SUBROUTINE DMPP0(MX,NPX,NX,AX,X,WX)                                 DGA00010
...
CCCCCCC                                                                 DGA00120
 ...
 1000 FORMAT(//,10X,'WARNING FROM DMP: MX MUST BE POSITIVE',//)         DGA00180
    ...
    5 L0=0                                                              DGA00200
      ...
      CALL DVKN0(NPX)                                                    DGA00240
      CALL GPT0(POINT,NPX)                                               DGA00250
      CALL GWT0(WEIGHT,NPX)                                              DGA00260
