...
void test_difference()
{
  ...
  string Crtstal_ID_str[crystal_nums] = {"994763", "080671", "050658", "030651", "160669", "040668", "994724", "050666", "010647", "010667"};
  stringstream line_string;
  double L, T;
  int count=0, pic_count=0, flag=0;
  int file_0[crystal_nums] = {227, 233, 230, 229, 226, 232, 225, 231, 228, 224};
  const char *CID;
  ...
  TLine *add_line_vert = new TLine(340, 0., 340, 3.);
  TCanvas *c1 = new TCanvas("c1", "c1", 720, 960);
  c1->Divide(3,4);
		...
		{ 
		  ...
		  if( (line.size() == 21) || (line.size() == 20) )
		      ...
		      if( (pic_count % 3) == 0 )
			{
			  ...
			  h1[count]->SetBinContent(n_bin, T);     // The origin crystal status
			}
		      if( (pic_count % 3) == 1 )
			{
			  int n_bin = h2[count]->FindBin(L);
			  h2[count]->SetBinContent(n_bin, T);     // The crystal after baked
			  Tb.push_back(T);
			  Tlambda.push_back(L);
			}
		      if( (pic_count % 3) == 2 )
			{
			  ...
			  h3[count]->SetBinContent(n_bin, T);     // The crystal after irridiated 10000 gry
			  Ta.push_back(T);
			}
