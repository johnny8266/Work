...
void test_difference()
{
  ...
  string Crtstal_ID_str[crystal_nums] = {"050666", "160669"};
  stringstream line_string;
  double L, T;
  int count=0, pic_count=0, flag=0;
  //  int file_0[crystal_nums] = {20, 21};  //2hr file num
  int file_0[crystal_nums] = {30, 31};   //24hr file num
  const char *CID;
  ...
  for(int i = 0 ; i < crystal_nums ; i++)
      ...
      for(int j = file_start ; j < (file_start + 3) ; j++)
	{
	  ...
	  data_file_path = "./after24h/" + ID + "/Sample"+ j +".Sample.Raw.asc";
