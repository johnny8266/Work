...
void pos_resolution()
{
  ...
  // string File_num[N_E_bin] = {"0.5", "1"};
  // double e_per_bin[N_E_bin] = {0.5, 1.};
  double e_statement[13] = {0.5, 1., 2., 3., 4., 5., 7., 10., 13., 18., 30., 50., 52.};
  */

  const int N_E_bin = 3;
  string File_num[N_E_bin] = {"-1.6", "-1.7", "-1.8"};
  double e_per_bin[N_E_bin] = {-1.6, -1.7, -1.8};
  double e_statement[13] = {0.5, 1., 2., 3., 4., 5., 7., 10., 13., 18., 30., 50., 52.};
  ...
  for( int i = 0 ; i < N_E_bin ; i++ )
	...
	{
	  if( i == 0)
	    root_file_name = "/vol0/pwang-l/Singularity/my_det/sub_glass/data/pos/g4eemc_glass_eval_mono_5.0_GeV_" + File_num[i] + ".root";
	  else
	    root_file_name = "/vol0/pwang-l/Singularity/my_det/sub_glass/data/pos/g4eemc_glass_eval_mono_5.0_GeV_" + File_num[i] + ".root";	  
	}
      */
      ...
      for(int j = 0 ; j < n_cluster ; j++)
	{
	  ...
	  if( j_eve == count )
	  ...
	  else
	    {
	      ...
	      if( clusterID == 1 )
		//		cout << count << " " << j_eve << " " << clusterID << " || " << pjx << " " << x_base << " " << z_base << " " << pri_Pz << endl;
		cout << diff_x << endl;

	      h1_posx_diff[i]->Fill(diff_x);
	      
	      /*
	      for(int k = 0 ; k < N_E_bin ; k++) // find the corresponding energy bin and fill
		{
		  if( (ge > (e_statement[k] - 0.1)) && ( ge < e_statement[k+1] ) )
		    {
		      h1_e_diff[k]->Fill(e_diff);
		      h1_posx_diff[k]->Fill(diff_x);
		      h1_posy_diff[k]->Fill(diff_y);
		      hprof_x[k]->Fill(x_base, diff_x);
		      hprof_y[k]->Fill(y_base, diff_y);
		    }
		}
	      */
  ...
  for(int i = 0 ; i < N_E_bin ; i++)
    {
      auto add_line = new TLine(-100, 0, 100, 0);
      c2->cd(i+1);
      hprof_x[i]->SetStats(0);
      hprof_x[i]->Draw();
      add_line->SetLineStyle(9);
      add_line->Draw("same");
    }
  */
