...
void close_event_sameE()
{
  ...
  for(int i = 0 ; i < N_E_bin ; i++)
    for(int j = 0 ; j < 8 ; j++)
      angle_range[i][j] = new TH1F(Form("angle_range_%d_%d", i, j), Form("angle_range_%d_%d", i, j), N_bin_plot, 0., (e_per_bin[i]*2.2) );
  ...
  for(int i = 0 ; i < N_E_bin ; i++)
      ...
      for(int j = 0 ; j < 8 ; j++)
	{
	  c1->cd(j+1);
	  gPad->SetLogy();
	  int mean_bin = e_per_bin[i] / (e_per_bin[i] * 2.2 / N_bin_plot);
	  ...
	  if( j > 5 )
	    {
	      int mean_separa = (e_per_bin[i] * 1.2) / ((e_per_bin[i]+10.) * 1.1 / N_bin_plot);
	      int low_e_count = angle_range[i][j]->Integral(1, mean_separa);
	      int high_e_count = angle_range[i][j]->Integral(mean_separa, 200);
  ...
  legend_e_reso = new TLegend(0.6, 0.45, 0.75, 0.65);
  ...
  for(int i = 0 ; i < N_E_bin ; i++)
    {
      ...
      angle_effi[i]->SetTitle("Efficiency of gamma separation");
	...
	angle_effi[i]->Draw("P same");
      ...
      legend_e_reso->AddEntry(Form("angle_effi_%d", i), Form("%.1f GeV", e_per_bin[i]), "p");
  ...
  for(int i = 0 ; i < N_E_bin ; i++)
    {
      ...
      TLegend *legend_Nclus_reco;
      legend_Nclus_reco = new TLegend(0.5, 0.6, 0.9, 0.85);
      legend_Nclus_reco->SetBorderSize(0);
      
      recons_2_angle[i]->GetXaxis()->SetTitle("#theta^{#circ}");
      recons_2_angle[i]->SetTitle(Form("Photons energy %.1f GeV", e_per_bin[i]));
      recons_2_angle[i]->SetStats(0);
      ...
      for(int j = 0 ; j < 2 ; j++)      
	{
      ...
      legend_Nclus_reco->AddEntry(Form("recons_1_angle_%d", i), "1 cluster reconstructed", "l");
      legend_Nclus_reco->AddEntry(Form("recons_2_angle_%d", i), "2 clusters reconstructed", "l");
      legend_Nclus_reco->Draw("same");
    }
